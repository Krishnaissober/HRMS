...
export function canRecordHiringDecision(currentCandidateStatus: string, decision: HiringDecisionType) {
  return HIRING_DECISION_TRANSITIONS[currentCandidateStatus]?.includes(decision) ?? false;
}

export const OFFER_STATUSES = ["DRAFT", "PENDING_APPROVAL", "APPROVED", "SENT", "VIEWED", "ACCEPTED", "DECLINED", "EXPIRED", "WITHDRAWN"] as const;
