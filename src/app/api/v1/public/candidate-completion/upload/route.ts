...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!token || !kinds.has(kind) || !(file instanceof File))
      throw validationError({ file: ["A valid identity image is required"] });
    if (!(["image/jpeg", "image/png"] as string[]).includes(file.type))
      throw validationError({ file: ["Only JPEG or PNG images are accepted"] });
    if (file.size < 1 || file.size > 5 * 1024 * 1024)
      throw validationError({ file: ["Identity images must be no larger than 5 MB"] });
    ...
    return successResponse(
      { objectKey, fileName: file.name, contentType: file.type, byteSize: file.size, kind },
      id,
    );
  } catch (error) {
