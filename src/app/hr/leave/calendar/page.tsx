...
export default function LeaveCalendarPage() {
  ...
  async function load(nextView = view) {
    const response = await fetch(`/api/v1/leave-calendar?view=${nextView}&date=${date}`);
    const result = await response.json();
    if (!response.ok) return setMessage(result.error?.message || "Could not load leave calendar");
  ...
  return (
    <main className="page-shell">
      <section className="panel">
        ...
        <div className="candidate-list" data-testid={`leave-${view}-calendar`}>
          {requests.map((request) => (
            <div className="candidate-row" key={request.id}>
              <span>
