...
function validateWindow(value: { scheduledStart?: string; scheduledEnd?: string; startsAt?: string; endsAt?: string }, context: z.RefinementCtx) {
  const start = value.scheduledStart || value.startsAt;
  const end = value.scheduledEnd || value.endsAt;
  if (start && end && new Date(end).getTime() <= new Date(start).getTime()) context.addIssue({ code: "custom", path: [value.scheduledEnd ? "scheduledEnd" : "endsAt"], message: "End must be after start" });
}
...
export const interviewUpdateSchema = z.object({
  ...
  if (value.status === "NO_SHOW" && !value.noShowReason?.trim()) {
    context.addIssue({ code: "custom", path: ["noShowReason"], message: "A reason is required when marking an interview as no-show" });
  }
});
