...
import { interviewCreateSchema, interviewEvaluationSchema, interviewTemplateSchema, interviewUpdateSchema } from "@/modules/interviews/schemas";
...
describe("interview scheduling", () => {
  it("requires a valid time window and an interviewer", () => {
    ...
    expect(() => interviewCreateSchema.parse({ ...valid, participantIds: [], scheduledEnd: valid.scheduledStart })).toThrow();
  });
  ...
  it("requires a reason when marking an interview as no-show", () => {
    expect(interviewUpdateSchema.parse({ status: "NO_SHOW", noShowReason: "Candidate did not attend" }).noShowReason).toBe("Candidate did not attend");
...
describe("interview evaluation", () => {
  it("accepts structured scores and supported recommendations", () => {
    expect(interviewEvaluationSchema.parse({ scores: { communication: 4 }, comments: "Clear explanation", recommendation: "HOLD" }).recommendation).toBe("HOLD");
    expect(() => interviewEvaluationSchema.parse({ scores: { communication: "4" } })).toThrow();
  });

  it("accepts reusable template questions", () => {
    expect(interviewTemplateSchema.parse({ name: "Technical", questions: [{ id: "q1", prompt: "Explain the approach" }] }).questions).toHaveLength(1);
  });
});
