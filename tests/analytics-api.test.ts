...
vi.mock("@/modules/analytics/service", () => ({ analyticsForDomain: mocks.analyticsForDomain, analyticsCsv: mocks.analyticsCsv }));
...
const context = { session: { user: { id: "user-a" } }, organizationId: "org-a", membership: { status: "ACTIVE" } };
const params = (domain: string) => ({ params: Promise.resolve({ domain }) });
const request = (path: string, organizationId = "org-a") => new NextRequest(`http://localhost${path}`, { headers: { "x-organization-id": organizationId } });
...
describe("Phase 11 analytics API contracts", () => {
  ...
  it("passes the authenticated tenant and validated filters to aggregation", async () => {
    const response = await analyticsGet(request("/api/v1/analytics/workforce?from=2026-01-01&department=People"), params("workforce"));
    expect(response.status).toBe(200);
    expect(mocks.requirePermissions).toHaveBeenCalledWith("user-a", "org-a", ["employees.read"]);
    expect(mocks.analyticsForDomain).toHaveBeenCalledWith("workforce", "org-a", { from: "2026-01-01", department: "People" }, { userId: "user-a", email: undefined });
  });
  ...
  it("rejects unauthenticated, unauthorized, cross-tenant, and invalid-filter requests", async () => {
    ...
    expect((await analyticsGet(request("/api/v1/analytics/leave"), params("leave"))).status).toBe(401);
    mocks.getAuthenticatedContext.mockResolvedValueOnce(context);
    mocks.requirePermissions.mockRejectedValueOnce(forbiddenError());
    expect((await analyticsGet(request("/api/v1/analytics/leave"), params("leave"))).status).toBe(403);
    mocks.getAuthenticatedContext.mockRejectedValueOnce(forbiddenError());
    expect((await analyticsGet(request("/api/v1/analytics/leave", "org-b"), params("leave"))).status).toBe(403);
    mocks.getAuthenticatedContext.mockResolvedValueOnce(context);
    expect((await analyticsGet(request("/api/v1/analytics/leave?from=2026-02-01&to=2026-01-01"), params("leave"))).status).toBe(422);
  });

  it("requires distinct export permission and audits the filtered CSV export", async () => {
    const response = await exportGet(request("/api/v1/reports/payroll/export?from=2026-01-01"), params("payroll"));
    expect(response.status).toBe(200);
    expect(response.headers.get("content-type")).toContain("text/csv");
    expect(mocks.requirePermissions).toHaveBeenCalledWith("user-a", "org-a", ["payroll.reports", "reports.export"]);
    expect(mocks.recordAuditEvent).toHaveBeenCalledWith(expect.objectContaining({ organizationId: "org-a", actorUserId: "user-a", action: "REPORT_EXPORTED", entityId: "payroll", metadata: { domain: "payroll", filters: { from: "2026-01-01" }, format: "CSV" } }));
  });
  ...
  it("does not generate or audit an export when export authorization fails", async () => {
    ...
    const response = await exportGet(request("/api/v1/reports/workforce/export"), params("workforce"));
