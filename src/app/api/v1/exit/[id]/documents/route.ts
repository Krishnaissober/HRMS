...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return successResponse(await listExitDocuments(c.organizationId, (await params).id), rid);
  } catch (e) {
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const body = parseBody(exitDocumentAttachSchema, await request.json());
    return successResponse(
      await attachExitDocument({
