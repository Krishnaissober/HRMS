...
export default function EmployeeAttendanceManagePage() {
  ...
  async function load(overrides?: { organizationId?: string; view?: View; date?: string }) {
    ...
    const response = await fetch(`/api/v1/attendance/employee/calendar?${params}`, {
      headers: { "x-organization-id": tenant },
    });
    const result = await response.json();
    if (!response.ok)
      return setMessage(result.error?.message || "Could not load attendance calendar");
  ...
  function navigate(direction: -1 | 1) {
    const next = moveDate(selectedDate, view, direction);
    setSelectedDate(next);
    void load({ date: next });
  }
  const title = useMemo(
    () => (calendar ? `${calendar.from} to ${calendar.to}` : selectedDate),
    ...
    <main className="page-shell">
      <section className="panel">
        ...
        <div className="toolbar" aria-label="Attendance calendar controls">
          ...
          <button type="button" onClick={() => navigate(-1)} aria-label="Previous period">
            Previous
          </button>
          <input
          ...
          />
          <button type="button" onClick={() => navigate(1)} aria-label="Next period">
            Next
          </button>
        </div>
        <div className="toolbar">
          ...
          >
            ...
            {EMPLOYEE_ATTENDANCE_STATUSES.map((item) => (
          ...
          <a href={`/hr/shifts?organizationId=${encodeURIComponent(organizationId)}`}>
            Shifts and rosters
          </a>
          <a href={`/hr/holidays?organizationId=${encodeURIComponent(organizationId)}`}>
            Holiday calendar
          </a>
        </div>
        <h2>
          {view[0].toUpperCase() + view.slice(1)} view · {title}
        </h2>
        {message && <p role="status">{message}</p>}
        <div
          className={`attendance-calendar attendance-calendar-${view}`}
          data-testid={`attendance-${view}-view`}
        >
          {calendar?.days.map((day) => (
            <article className="panel" key={day.date} data-date={day.date}>
              <h3>
              ...
              {day.holidays.map((holiday) => (
                <p key={holiday.id}>Holiday · {holiday.name}</p>
              ))}
              {!day.attendance.length && !day.holidays.length && <p>No attendance</p>}
              {day.attendance.map((item) => (
                <div className="candidate-row" key={item.id}>
                  <span>
                    {item.employee.employeeNo} · {item.employee.firstName} {item.employee.lastName}
                  </span>
                  <span>
                    {item.status} · {item.durationMinutes ?? 0} min
                  </span>
                  <span>
                    {item.shift
                      ? `${item.shift.name} ${item.shift.startTime}–${item.shift.endTime} ${item.shift.timezone}`
                      : "No shift"}
                  </span>
