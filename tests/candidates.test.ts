...
describe("candidate intake validation", () => {
  ...
  it("rejects mismatched upload metadata", () => {
    expect(() =>
      validateDocumentMetadata({ fileName: "resume.exe", contentType: "application/pdf" }),
    ).toThrow();
    expect(() =>
      validateDocumentMetadata({ fileName: "resume.pdf", contentType: "application/pdf" }),
    ).not.toThrow();
  });
});
...
describe("public candidate API", () => {
  it("returns a consistent validation response", async () => {
    const request = new NextRequest("http://localhost/api/v1/public/candidates", {
      method: "POST",
