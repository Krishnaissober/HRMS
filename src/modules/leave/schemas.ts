...
export const leaveTypeSchema = z
  .object({
    ...
    eligibility: z
      ...
      .nullable(),
    accrualPolicy: z
      .object({ frequency: z.enum(["NONE", "YEARLY", "MONTHLY"]).default("NONE") })
      .optional()
      .nullable(),
  ...
  .refine((value) => value.carryForwardEnabled || value.maxCarryForwardDays == null, {
    message: "Carry-forward must be enabled before setting a limit",
...
export const carryForwardSchema = z
  .object({
    employeeId: z.string().min(1),
    leaveTypeId: z.string().min(1),
    fromYear: z.number().int().min(2000).max(2200),
    toYear: z.number().int().min(2000).max(2200),
  })
  .refine((value) => value.toYear === value.fromYear + 1, {
    message: "Carry-forward must target the next year",
    path: ["toYear"],
  });
export const leaveRequestSchema = z
  .object({
    ...
    startDate: dateOnly,
    endDate: dateOnly,
    durationType: z.enum(["FULL_DAY", "HALF_DAY"]).default("FULL_DAY"),
    reason: z.string().trim().min(1).max(2000),
    attachmentObjectKey: z.string().trim().max(500).optional(),
    attachmentFileName: z.string().trim().max(255).optional(),
    attachmentContentType: z.enum(["application/pdf", "image/jpeg", "image/png"]).optional(),
    attachmentByteSize: z
      .number()
      .int()
      .positive()
      .max(25 * 1024 * 1024)
  ...
  .refine((value) => value.durationType !== "HALF_DAY" || value.startDate === value.endDate, {
  ...
  .refine(
    (value) => {
      const fields = [
        value.attachmentObjectKey,
        value.attachmentFileName,
        value.attachmentContentType,
        value.attachmentByteSize,
      ];
      return fields.every(Boolean) || fields.every((item) => item == null);
    },
    { message: "Complete attachment metadata is required", path: ["attachmentObjectKey"] },
  );
...
export const leaveListSchema = z
  .object({
