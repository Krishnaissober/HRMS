...
} from "@/modules/employee-attendance/constants";
...
export const shiftCreateSchema = z.object({
  ...
  weeklyOffs,
  rotationPattern: z.string().trim().max(500).optional().nullable(),
...
export const shiftAssignmentSchema = z.object({
  ...
  weeklyOffs,
  rotationCode: z.string().trim().max(100).optional().nullable(),
  active: z.boolean().default(true),
});
...
export const correctionRequestSchema = z
  .object({
    ...
    requestedCheckInAt: dateTime.optional().nullable(),
    requestedCheckOutAt: dateTime.optional().nullable(),
    reason: z.string().trim().min(1).max(2000),
  })
  .refine((value) => value.requestedCheckInAt || value.requestedCheckOutAt, {
...
export const overtimeSchema = z.object({
  approvedOvertimeMinutes: z.number().int().min(0).max(1440),
});
