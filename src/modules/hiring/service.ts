...
import { getOffer, hiringDecisionInclude } from "@/modules/hiring/repository";
...
export async function createHiringDecision(input: {
  ...
  return db.$transaction(async (tx) => {
    ...
    if (!canRecordHiringDecision(application.candidate.status, input.decision))
      throw new AppError(
        "CONFLICT",
        `The ${input.decision} decision is not valid from candidate status ${application.candidate.status}`,
        409,
      );
    const nextStatus =
      input.decision === "HIRE" ? "SELECTED" : input.decision === "HOLD" ? "HOLD" : "REJECTED";
    ...
    if (candidateUpdate.count !== 1)
      throw new AppError(
        "CONFLICT",
        "The candidate status changed while the decision was being recorded",
        409,
      );
    ...
    if (applicationUpdate.count !== 1)
      throw new AppError(
        "CONFLICT",
        "The application status changed while the decision was being recorded",
        409,
      );
    ...
    await tx.candidateActivity.create({
      data: {
        ...
        candidateId: application.candidateId,
        actorUserId: input.actorUserId,
        action: "HIRING_DECISION_RECORDED",
        fromStatus: application.candidate.status,
        toStatus: nextStatus,
        note: input.notes?.trim() || input.reason?.trim(),
        metadata: {
          applicationId: application.id,
          decision: input.decision,
...
export async function createOfferTemplate(input: {
  ...
  const template = await db.$transaction(async (tx) => {
    const result = await tx.offerTemplate.create({
      data: {
        organizationId: input.organizationId,
        name: input.name,
        body: input.body,
        approvalRequired: input.approvalRequired,
        approvalSteps: input.approvalSteps as PrismaTypes.InputJsonValue,
...
export async function createOffer(input: {
  ...
  try {
    return await db.$transaction(async (tx) => {
      const decision = await tx.hiringDecision.findFirst({
        where: {
          id: input.hiringDecisionId,
          organizationId: input.organizationId,
          applicationId: input.applicationId,
          decision: "HIRE",
        },
      });
      const template = await tx.offerTemplate.findFirst({
      ...
      const existingOffer = await tx.offer.findUnique({
        where: { hiringDecisionId: decision.id },
        select: { id: true },
      });
      if (existingOffer)
        throw new AppError("CONFLICT", "An offer already exists for this hiring decision", 409);
      const steps = template.approvalRequired
        ? parseSteps(template.approvalSteps).length
          ? parseSteps(template.approvalSteps)
          : ["Approval"]
        : [];
      ...
      await writeAuditEvent(tx, {
        ...
        entityId: offer.id,
        requestId: input.requestId,
        metadata: { applicationId: application.id, hiringDecisionId: decision.id, status },
      });
      return offer;
    });
  } catch (error) {
...
export async function approveOffer(input: {
  ...
  return db.$transaction(async (tx) => {
    ...
    if (offer.status !== "PENDING_APPROVAL")
      throw validationError({ status: ["Only pending offers can be approved"] });
    ...
    const remaining = offer.approvals.some(
      (approval) => approval.id !== pending.id && approval.status === "PENDING",
    );
    const updated = await tx.offer.update({
      where: { id: offer.id },
      data: { status: remaining ? "PENDING_APPROVAL" : "APPROVED" },
    });
    await writeAuditEvent(tx, {
      organizationId: input.organizationId,
      actorUserId: input.actorUserId,
      ...
      entityId: offer.id,
      requestId: input.requestId,
      metadata: { stepOrder: pending.stepOrder, remaining },
    });
    return updated;
  });
}
...
export async function sendOffer(input: {
  ...
  if (!["APPROVED", "SENT"].includes(offer.status))
    throw validationError({ status: ["Only approved offers or failed deliveries can be sent"] });
  if (offer.expiryDate && offer.expiryDate <= new Date())
    throw validationError({ expiryDate: ["The offer has expired"] });
  const existingDelivery = offer.deliveries[0];
  if (offer.status === "SENT" && existingDelivery?.status !== "FAILED")
    return {
      offer,
      delivery: existingDelivery,
      deliveryStatus: existingDelivery?.status || "QUEUED",
    };
  ...
  const queued = await db.$transaction(async (tx) => {
    if (existingDelivery?.status === "FAILED") {
      const updated = await tx.offer.update({
        where: { id: offer.id },
        data: { responseTokenHash: hashToken(token), sentAt: new Date() },
      });
      const deliveryUpdate = await tx.offerDelivery.updateMany({
        where: { id: existingDelivery.id, status: "FAILED" },
        data: {
          status: "QUEUED",
      ...
      if (deliveryUpdate.count !== 1)
        throw new AppError("CONFLICT", "Offer delivery is already being retried", 409);
      const delivery = await tx.offerDelivery.findUniqueOrThrow({
        where: { id: existingDelivery.id },
      });
      await writeAuditEvent(tx, {
        organizationId: input.organizationId,
        actorUserId: input.actorUserId,
        action: "OFFER_SEND_RETRY_QUEUED",
        entityType: "Offer",
        entityId: offer.id,
        requestId: input.requestId,
      });
    ...
    const updatedCount = await tx.offer.updateMany({
      where: { id: offer.id, organizationId: input.organizationId, status: "APPROVED" },
      data: { status: "SENT", sentAt: new Date(), responseTokenHash: hashToken(token) },
    });
    if (updatedCount.count !== 1)
      throw new AppError("CONFLICT", "The offer changed while send was being queued", 409);
    const updated = await tx.offer.findUniqueOrThrow({ where: { id: offer.id } });
    const delivery = await tx.offerDelivery.create({
      data: {
        organizationId: input.organizationId,
        offerId: offer.id,
        idempotencyKey: `offer-send:${offer.id}`,
      },
    });
    await writeAuditEvent(tx, {
      organizationId: input.organizationId,
  ...
  try {
    const deliveryResult = await emailProvider().send({
      recipient: offer.candidate.email,
      subject: `Offer for ${offer.application.requisition.title}`,
      body: `Please review your offer and respond here: ${responseUrl}`,
      template: "OFFER_AVAILABLE",
    });
    const delivery = await db.$transaction(async (tx) => {
      const result = await tx.offerDelivery.update({
        where: { id: queued.delivery.id },
        data: {
          status: "SUCCEEDED",
      ...
      await writeAuditEvent(tx, {
        organizationId: input.organizationId,
        actorUserId: input.actorUserId,
        action: "OFFER_DELIVERY_SUCCEEDED",
        entityType: "Offer",
        entityId: offer.id,
        requestId: input.requestId,
      });
...
export async function respondToOffer(input: {
  ...
  return db.$transaction(async (tx) => {
    const offer = await tx.offer.findFirst({
      where: { responseTokenHash: hashToken(input.token) },
      include: { candidate: true },
    });
    if (!offer) throw notFoundError();
    if (!["SENT", "VIEWED"].includes(offer.status))
      throw validationError({ status: ["This offer is not available for response"] });
    if (offer.expiryDate && offer.expiryDate <= new Date()) {
    ...
    const updated = await tx.offer.update({
      ...
      data: {
        ...
        responseNotes: input.notes?.trim() || null,
    ...
    await writeAuditEvent(tx, {
      organizationId: offer.organizationId,
      action: `OFFER_${input.response}`,
      entityType: "Offer",
      entityId: offer.id,
      requestId: input.requestId,
      metadata: { candidateId: offer.candidateId },
    });
    return updated;
  });
}
...
export async function viewOffer(input: { token: string; requestId?: string }) {
  return db.$transaction(async (tx) => {
    ...
    if (!offer) throw notFoundError();
    if (!["SENT", "VIEWED"].includes(offer.status))
      throw validationError({ status: ["This offer is not available to view"] });
    const updated =
      offer.status === "SENT"
    ...
    return {
      ...
      position: offer.application.requisition.title,
...
export async function generateOfferPdf(organizationId: string, id: string) {
  ...
  if (["DRAFT", "PENDING_APPROVAL"].includes(offer.status))
    throw new AppError("FORBIDDEN", "The offer is not available for download", 403);
  return offer;
}
