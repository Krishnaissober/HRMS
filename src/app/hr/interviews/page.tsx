...
export default function InterviewListPage() {
  ...
  const scheduled = items.filter((i) => ["SCHEDULED", "RESCHEDULED", "CHECKED_IN"].includes(i.status));
  const completed = items.filter((i) => i.status === "COMPLETED");
  const toBeScheduled = candidates.filter((c) => ["APPLIED", "SCREENING", "SHORTLISTED", "INTERVIEW"].includes(c.status) && c.interviews.length === 0);
    ...
    <main className="page-shell space-y-6 pb-12">
        ...
        <div className="relative z-10 flex flex-col gap-6">
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
            <div className="space-y-2">
              <p className="text-xs font-extrabold uppercase tracking-widest text-indigo-300">Interview Management</p>
              <h1 className="text-2xl md:text-3xl font-black tracking-tight text-white">Interviews</h1>
            ...
            <Link href="/hr/interviews/new" className="inline-flex items-center gap-2 rounded-2xl bg-indigo-500 hover:bg-indigo-400 border border-indigo-400/40 px-4 py-2.5 text-xs font-bold text-white shadow-lg transition-all active:scale-95 shrink-0 self-start">
          ...
          <div className="flex gap-3">
            ...
            <button type="button" onClick={() => void load()} className="inline-flex items-center gap-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 px-5 py-2.5 text-sm font-bold text-white transition-colors">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          <div key={label} className="group relative overflow-hidden rounded-2xl border border-border/60 bg-card p-5 shadow-sm">
            <div className="flex items-center justify-between">
              ...
              <div className={cn("flex h-9 w-9 items-center justify-center rounded-xl",
                color === "amber" ? "bg-amber-50 dark:bg-amber-950/60 text-amber-600 dark:text-amber-400" :
            ...
            <span className="mt-3 block text-3xl font-black tracking-tight text-foreground">{count}</span>
          </div>
        ))}
      </div>
      ...
      <section className="rounded-3xl border border-border/60 bg-card shadow-sm overflow-hidden">
        <div className="flex items-center justify-between border-b border-border/50 px-6 py-4">
          <div>
            <p className="text-xs font-extrabold uppercase tracking-wider text-amber-600 dark:text-amber-400">Awaiting Scheduling</p>
            <h2 className="text-lg font-extrabold text-foreground">To Be Scheduled</h2>
          </div>
          <span className="inline-flex items-center justify-center h-7 w-7 rounded-lg bg-amber-100 dark:bg-amber-950/60 text-amber-700 dark:text-amber-300 text-xs font-black">{toBeScheduled.length}</span>
        </div>
        <div className="p-6 space-y-2.5">
          {!toBeScheduled.length && <p className="text-sm text-muted-foreground font-medium p-2">No candidates are waiting for an interview.</p>}
          {toBeScheduled.map((candidate) => (
            <div key={candidate.id} className="group flex items-center gap-4 rounded-2xl border border-border/50 bg-background/60 p-4 transition-all duration-200 hover:-translate-y-0.5 hover:border-amber-500/40 hover:bg-card hover:shadow-md">
              ...
              <div className="flex-1 min-w-0">
                <span className="block font-extrabold text-sm text-foreground truncate">{candidate.firstName} {candidate.lastName}</span>
                <span className="block text-xs text-muted-foreground font-medium truncate">{candidate.referenceNo} · {candidate.roleOfInterest}</span>
              </div>
              <span className={cn("inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase shrink-0", CANDIDATE_STATUS_COLORS[candidate.status] || "bg-muted text-muted-foreground")}>
                {candidate.status}
              </span>
              <div className="flex items-center gap-2 shrink-0">
                <a href={`/hr/candidates/${candidate.id}/preview`} className="inline-flex items-center gap-1.5 rounded-xl border border-border/60 bg-card hover:bg-muted px-3 py-1.5 text-xs font-bold text-foreground transition-colors">
                  Preview
                </a>
                <a href={`/hr/interviews/new?candidateId=${encodeURIComponent(candidate.id)}&applicationId=${encodeURIComponent(candidate.applications[0]?.id || "")}`} className="inline-flex items-center gap-1.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 px-3 py-1.5 text-xs font-bold text-white transition-colors">
      ...
      <section className="rounded-3xl border border-border/60 bg-card shadow-sm overflow-hidden">
        <div className="flex items-center justify-between border-b border-border/50 px-6 py-4">
          <div>
            <p className="text-xs font-extrabold uppercase tracking-wider text-indigo-600 dark:text-indigo-400">Upcoming</p>
            <h2 className="text-lg font-extrabold text-foreground">Scheduled Interviews</h2>
          </div>
          <span className="inline-flex items-center justify-center h-7 w-7 rounded-lg bg-indigo-100 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300 text-xs font-black">{scheduled.length}</span>
        </div>
        <div className="p-6 space-y-2.5">
          {!scheduled.length && <p className="text-sm text-muted-foreground font-medium p-2">No scheduled interviews.</p>}
          {scheduled.map((interview) => (
            <Link key={interview.id} href={`/hr/interviews/${interview.id}`} className="group flex items-center gap-4 rounded-2xl border border-border/50 bg-background/60 p-4 transition-all duration-200 hover:-translate-y-0.5 hover:border-indigo-500/40 hover:bg-card hover:shadow-md">
              ...
              <div className="flex-1 min-w-0">
                <span className="block font-extrabold text-sm text-foreground group-hover:text-indigo-600 transition-colors truncate">{interview.candidate.firstName} {interview.candidate.lastName}</span>
                <span className="block text-xs text-muted-foreground font-medium">{interview.referenceNo}</span>
              </div>
              <div className="hidden sm:block text-right shrink-0">
                <span className="block text-xs font-semibold text-foreground">{new Date(interview.scheduledStart).toLocaleDateString("en-US", { month: "short", day: "numeric" })}</span>
                <span className="block text-xs text-muted-foreground">{new Date(interview.scheduledStart).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" })} · {interview.timezone}</span>
              </div>
              <span className={cn("inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase shrink-0", STATUS_COLORS[interview.status] || "bg-muted text-muted-foreground")}>
      ...
      <section className="rounded-3xl border border-border/60 bg-card shadow-sm overflow-hidden">
        <div className="flex items-center justify-between border-b border-border/50 px-6 py-4">
          <div>
            <p className="text-xs font-extrabold uppercase tracking-wider text-emerald-600 dark:text-emerald-400">Done</p>
            <h2 className="text-lg font-extrabold text-foreground">Completed Interviews</h2>
          </div>
          <span className="inline-flex items-center justify-center h-7 w-7 rounded-lg bg-emerald-100 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 text-xs font-black">{completed.length}</span>
        </div>
        <div className="p-6 space-y-2.5">
          {!completed.length && <p className="text-sm text-muted-foreground font-medium p-2">No completed interviews.</p>}
          {completed.map((interview) => (
            <Link key={interview.id} href={`/hr/interviews/${interview.id}`} className="group flex items-center gap-4 rounded-2xl border border-border/50 bg-background/60 p-4 transition-all duration-200 hover:-translate-y-0.5 hover:border-emerald-500/40 hover:bg-card hover:shadow-md">
              ...
              <div className="flex-1 min-w-0">
                <span className="block font-extrabold text-sm text-foreground group-hover:text-emerald-600 transition-colors truncate">{interview.candidate.firstName} {interview.candidate.lastName}</span>
                <span className="block text-xs text-muted-foreground font-medium">{interview.referenceNo}</span>
              </div>
              <div className="hidden sm:block text-right shrink-0">
                <span className="block text-xs font-semibold text-foreground">{new Date(interview.scheduledStart).toLocaleDateString("en-US", { month: "short", day: "numeric" })}</span>
