...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const allowOrganizationWide = await hasPermission(
      c.session.user.id,
      c.organizationId,
      EMPLOYEE_PERMISSIONS.exitManage,
    );
    const allowTeam = await hasPermission(
      c.session.user.id,
      c.organizationId,
      EMPLOYEE_PERMISSIONS.exitTeamManage,
    );
    if (!allowOrganizationWide && !allowTeam) throw forbiddenError();
    const body = parseBody(exitCreateSchema, await request.json());
    return successResponse(
      await createExitCase({
