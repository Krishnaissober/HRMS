...
vi.mock("@/lib/db", () => ({ db: { membership: { findUnique: mocks.membership }, employee: { findFirst: mocks.employee, findMany: mocks.reports } } }));
...
describe("Phase 11 actor data scope", () => {
  ...
  it("limits a manager to direct reports and rejects an out-of-scope employee", async () => {
    ...
    const scope = await resolveAnalyticsScope("org-a", { userId: "manager-user", email: "manager@example.test" });
    ...
    expect(() => scopedEmployeeWhere({ employeeId: "outside" }, scope)).toThrow(forbiddenError().message);
  });

  it("fails closed for a recruiter when no persisted requisition assignment exists", async () => {
    mocks.membership.mockResolvedValue({ roles: [{ role: { slug: "recruiter", name: "Recruiter" } }] });
    expect(await resolveAnalyticsScope("org-a", { userId: "recruiter-user", email: "recruiter@example.test" })).toEqual({ recruiterRestricted: true });
  });

  it("does not constrain established HR roles", async () => {
    mocks.membership.mockResolvedValue({ roles: [{ role: { slug: "hr-manager", name: "HR Manager" } }] });
    expect(await resolveAnalyticsScope("org-a", { userId: "hr-user", email: "hr@example.test" })).toEqual({});
  });
});
