...
export async function getAuthenticatedContext(request?: Request) {
  ...
  const membership = requestedOrganizationId
    ? await db.membership.findUnique({ where: { organizationId_userId: { organizationId: requestedOrganizationId, userId: session.user.id } } })
    : await db.membership.findFirst({ where: { userId: session.user.id, status: "ACTIVE" }, orderBy: { createdAt: "asc" } });
