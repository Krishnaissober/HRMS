...
export default async function globalSetup() {
  ...
  const signUpResponse = await fetch(`${baseUrl}/api/auth/sign-up/email`, {
    ...
    body: JSON.stringify({ name: "Phase 3 E2E User", email, password }),
  });
  if (!signUpResponse.ok && ![400, 409, 422].includes(signUpResponse.status))
    throw new Error(
      `Could not create E2E auth user: ${signUpResponse.status} ${await signUpResponse.text()}`,
    );
  ...
  const approverSignUp = await fetch(`${baseUrl}/api/auth/sign-up/email`, {
    method: "POST",
    headers: { "content-type": "application/json", origin: trustedOrigin },
    body: JSON.stringify({
      name: "Phase 7 Leave Approver",
      email: approverEmail,
      password: approverPassword,
    }),
  });
  if (!approverSignUp.ok)
    throw new Error(
      `Could not create Phase 7 approver: ${approverSignUp.status} ${await approverSignUp.text()}`,
    );
  const approverUser = await db.user.findUniqueOrThrow({ where: { email: approverEmail } });
  const organization = await db.organization.create({
    data: { name: `Phase 3 E2E ${runId}`, slug: `phase3-e2e-${runId}` },
  });
  const otherOrganization = await db.organization.create({
    data: { name: `Phase 3 E2E Other ${runId}`, slug: `phase3-e2e-other-${runId}` },
  });
  const permissionNames = [
    ...
    "employees.assets.manage",
    "employees.access.manage",
    "employees.mentor.manage",
    "attendance.read",
    "attendance.check-in",
  ...
  const permissions = await Promise.all(
    permissionNames.map((name) =>
      db.permission.upsert({
        where: { name },
        update: {},
        create: { name, description: "Phase 3 E2E permission" },
      }),
    ),
  );
  const phase8Permissions = await Promise.all(
    [
      "documents.read",
      "documents.manage",
      "notifications.read",
      ...
      db.permission.upsert({
        where: { name },
        update: {},
        create: { name, description: "Phase 8/9/11/12 E2E permission" },
      }),
    ),
  );
  ...
  async function addMembership(organizationId: string) {
    const role = await db.role.create({
      data: { organizationId, name: "Phase 3 E2E HR", slug: `phase3-e2e-hr-${runId}` },
    });
    await db.rolePermission.createMany({
      data: permissions.map((permission) => ({ roleId: role.id, permissionId: permission.id })),
    });
    const membership = await db.membership.create({
      data: { organizationId, userId: e2eUser.id, status: "ACTIVE" },
    });
  ...
  async function addApproverMembership(organizationId: string) {
    const role = await db.role.create({
      data: {
        organizationId,
        name: "Phase 7 E2E Leave Approver",
        slug: `phase7-e2e-approver-${runId}`,
      },
    });
    const approverPermissions = permissions.filter((permission) =>
      ["leave.read", "leave.approve"].includes(permission.name),
    );
    await db.rolePermission.createMany({
      data: approverPermissions.map((permission) => ({
        roleId: role.id,
        permissionId: permission.id,
      })),
    });
    const membership = await db.membership.create({
      data: { organizationId, userId: approverUser.id, status: "ACTIVE" },
    });
    await db.membershipRole.create({ data: { membershipId: membership.id, roleId: role.id } });
  }
  ...
  const requisition = await db.jobRequisition.create({
    data: {
      ...
      referenceNo: `REQ-E2E-${runId}`,
      title: "Phase 3 E2E Role",
      status: "PUBLISHED",
      approvedAt: new Date(),
      openedAt: new Date(),
    },
  ...
  const candidate = await db.candidate.create({
    data: {
      organizationId: organization.id,
      referenceNo: `CAND-E2E-${runId}`,
      firstName: "Phase",
      lastName: "Candidate",
      email: `candidate-${runId}@example.test`,
      phone: `900${String(Date.now()).slice(-7)}`,
  ...
  return async () => {
    ...
    if (email.startsWith("phase3-e2e-") && email.endsWith("@example.test"))
      await db.user.delete({ where: { id: e2eUser.id } }).catch(() => undefined);
