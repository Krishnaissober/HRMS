...
export default function PayrollPage() {
  ...
  const inputCls =
    "w-full rounded-xl border border-border/60 bg-background px-4 py-2.5 text-sm font-medium text-foreground placeholder:text-muted-foreground focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition-colors";
  const labelCls =
    "block text-xs font-extrabold uppercase tracking-wider text-muted-foreground mb-1.5";
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <section className="rounded-3xl border border-border/60 bg-card p-6 shadow-sm space-y-5">
          ...
          <div className="space-y-4">
            ...
            <button
              onClick={() => void salary()}
              className="inline-flex items-center gap-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 px-5 py-2.5 text-sm font-bold text-white transition-colors"
            >
        ...
        <section className="rounded-3xl border border-border/60 bg-card p-6 shadow-sm space-y-5">
          ...
          <button
            onClick={() => void run()}
            className="inline-flex items-center gap-2 rounded-xl bg-amber-600 hover:bg-amber-700 px-5 py-2.5 text-sm font-bold text-white transition-colors"
          >
              ...
              <div className="flex items-center gap-2 shrink-0">
                {r.status === "DRAFT" && (
                  <button
                    ...
                    className="rounded-xl bg-blue-600 hover:bg-blue-700 px-3 py-1.5 text-xs font-bold text-white transition-colors"
                  >
                    Prepare
                  </button>
                )}
                {r.status === "PREPARED" && (
                  <button
                    ...
                    className="rounded-xl bg-amber-600 hover:bg-amber-700 px-3 py-1.5 text-xs font-bold text-white transition-colors"
                  >
                    Review
                  </button>
                )}
                {r.status === "REVIEWED" && (
                  <button
                    ...
                    className="rounded-xl bg-emerald-600 hover:bg-emerald-700 px-3 py-1.5 text-xs font-bold text-white transition-colors"
                  >
                    Approve
                  </button>
                )}
      ...
      <section className="rounded-3xl border border-border/60 bg-card shadow-sm overflow-hidden">
        ...
        <div className="p-6 space-y-2.5">
          ...
          {expenses.map((e) => (
            <article
              key={e.id}
              className="flex items-center gap-4 rounded-2xl border border-border/50 bg-background/60 p-4 transition-all hover:border-rose-500/40 hover:bg-card hover:shadow-md"
            >
