  ...
  try {
    ...
    const [user, membership, unreadNotifications] = await Promise.all([
      ...
      db.membership.findUniqueOrThrow({
        where: { organizationId_userId: { organizationId: context.organizationId, userId: context.session.user.id } },
        include: { roles: { include: { role: { select: { name: true, slug: true } } } } },
      }),
      db.appNotification.count({ where: { organizationId: context.organizationId, userId: context.session.user.id, readAt: null } }),
    ]);
