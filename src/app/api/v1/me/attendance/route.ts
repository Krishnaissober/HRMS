...
export async function GET(request: NextRequest) { const id = requestId(request); try { const context = await getAuthenticatedContext(request); await requirePermission(context.session.user.id, context.organizationId, EMPLOYEE_ATTENDANCE_PERMISSIONS.read); return successResponse(await listSelfAttendance({ organizationId: context.organizationId, userEmail: context.session.user.email, query: parseQuery(attendanceListSchema, request.nextUrl.searchParams) }), id); } catch (error) { return errorResponse(error, id); } }
