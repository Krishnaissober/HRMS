...
export async function GET(
  ...
  try {
    ...
    await requirePermission(
      ...
      CANDIDATE_PERMISSIONS.documentsRead,
    );
    const values = await params;
    const result = await getCandidateDocumentUrl({
    ...
    if (request.nextUrl.searchParams.get("response") === "json")
      return successResponse({ downloadUrl: result.url }, id);
