...
type Item = { id: string; status: string; employee: { employeeNo: string; firstName: string; lastName: string; status: string }; template: { name: string }; tasks: Task[]; documents: Array<{ status: string }> };
...
export default function OnboardingPage() {
  ...
  const load = useCallback(async () => {
    ...
    const response = await fetch(`/api/v1/onboarding?page=1&pageSize=100${filter === "ALL" ? "" : `&status=${filter}`}`);
  ...
  const summary = useMemo(() => items.reduce((result, item) => {
    ...
    return result;
  }, { total: 0, completed: 0, tasks: 0, taskTotal: 0, overdue: 0 }), [items]);
      ...
      <section className="relative overflow-hidden rounded-3xl border border-indigo-500/20 bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 p-6 md:p-8 text-white shadow-2xl">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-start justify-between gap-6">
          <div className="space-y-2">
            ...
            <h1 className="text-2xl md:text-3xl font-black tracking-tight text-white">Onboarding</h1>
            <p className="text-sm text-indigo-100/75 font-medium max-w-lg">
              Prepare new employees before day one, guide their first 90 days, and keep every owner accountable.
            </p>
          </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="rounded-2xl border border-border/60 bg-card p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-extrabold text-muted-foreground uppercase tracking-wider">Active Plans</span>
          ...
          <span className="mt-3 block text-3xl font-black tracking-tight text-foreground">{summary.total - summary.completed}</span>
          <span className="text-xs text-muted-foreground font-medium">{summary.completed} completed</span>
        </div>
        <div className="rounded-2xl border border-border/60 bg-card p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-extrabold text-muted-foreground uppercase tracking-wider">Task Progress</span>
          ...
          <span className="mt-3 block text-3xl font-black tracking-tight text-foreground">{summary.tasks}<span className="text-lg text-muted-foreground font-bold">/{summary.taskTotal}</span></span>
          <span className="text-xs text-muted-foreground font-medium">{summary.overdue} overdue</span>
        </div>
        <div className="rounded-2xl border border-border/60 bg-card p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-extrabold text-muted-foreground uppercase tracking-wider">Completion Rate</span>
          ...
          <div className="p-6 space-y-2.5">
            {items.length === 0 && (
              <div className="rounded-2xl border border-border/40 bg-muted/30 p-10 text-center">
                ...
                <p className="text-sm font-semibold text-muted-foreground">No onboarding plans match this filter.</p>
              </div>
            )}
            {items.map((item) => {
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 space-y-2">
                    <div className="flex items-center gap-3 flex-wrap">
                      ...
                      <span className={cn("inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-black uppercase shrink-0", STATUS_COLORS[item.status] || "bg-muted text-muted-foreground")}>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-[11px] font-bold text-muted-foreground shrink-0">{percent}%</span>
                    </div>
                    <span className="block text-xs text-muted-foreground font-medium">{item.employee.employeeNo} · {item.template.name} · {done}/{item.tasks.length} tasks · {documents} verified docs</span>
                  </div>
