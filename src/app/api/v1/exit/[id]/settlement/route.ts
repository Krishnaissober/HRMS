...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await requirePermission(c.session.user.id, c.organizationId, EMPLOYEE_PERMISSIONS.exitSettle);
    const b = parseBody(exitSettlementSchema, await request.json());
    return successResponse(
      await updateExitSettlement({
