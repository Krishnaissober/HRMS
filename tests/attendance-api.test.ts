...
const {
  ...
  checkInCandidate: vi.fn(),
  checkOutCandidate: vi.fn(),
  recordAttendanceException: vi.fn(),
  listVisits: vi.fn(),
...
describe("candidate attendance API authorization", () => {
  beforeEach(() => {
    ...
    checkOutCandidate.mockResolvedValue({
      id: "visit-1",
      organizationId: "org-a",
      status: "CHECKED_OUT",
    });
    listVisits.mockResolvedValue({ items: [], total: 0 });
  });
  ...
  it("rejects unauthenticated and unauthorized check-in", async () => {
    ...
    expect(
      (await checkIn(request("/api/v1/attendance/check-in", { candidateId: "candidate-1" })))
        .status,
    ).toBe(401);
    getAuthenticatedContext.mockResolvedValue(context);
  ...
  it("passes tenant context to check-in, check-out and list paths", async () => {
    ...
    expect(
      (await checkOut(request("/api/v1/attendance/check-out", { visitId: "visit-1" }))).status,
    ).toBe(200);
    expect((await list(request("/api/v1/attendance?page=1&pageSize=10"))).status).toBe(200);
    expect(checkInCandidate).toHaveBeenCalledWith(
      expect.objectContaining({ organizationId: "org-a", actorUserId: "user-a" }),
    );
    expect(checkOutCandidate).toHaveBeenCalledWith(
      expect.objectContaining({ organizationId: "org-a", actorUserId: "user-a" }),
    );
