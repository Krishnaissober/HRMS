...
export async function POST(request: NextRequest) {
  const id = requestId(request);
  try { const context = await getAuthenticatedContext(request); await requirePermission(context.session.user.id, context.organizationId, ATTENDANCE_PERMISSIONS.manage); const parsed = parseBody(visitorCreateSchema, await request.json()); return successResponse(await registerVisitor({ organizationId: context.organizationId, actorUserId: context.session.user.id, ...parsed, requestId: id }), id, { status: 201 }); } catch (error) { return errorResponse(error, id); }
}
